...
| 维度 | 偏激种子数 | 语料覆盖 | 取舍 |
|---|---:|---:|---|
| 夸张修辞度 | 24 | **3.7%** | 入选：三项判据都最稀缺 |
| 情绪极端度 | 47 | **14.4%** | 入选：覆盖次低 |
| 确定性强度 | 23 | 19.5% | 不入选 |
| 绝对化/最高级 | 44 | 64.5% | 不入选 |
| 程度加强度 | 26 | 73.3% | 不入选 |
...
| `resources/种子审核结论.json` | **人工审核裁定**：是否收入 / 子类 / 理由（当前为 `{}`，待你填） |

### 6.5 当前待审核候选 17 条

`output/稀缺维度种子候选_待审核.csv`。按建议子类分组：

| 建议子类 | 一致度 | 候选词（语料频数） |
|---|---|---|
| 夸张修辞 | 3/3 | 跨越式(285) 飞跃(71) 得天独厚(60) 霸主(58) 跳跃式(15) 滚雪球(9) |
| 夸张修辞 | 2/3 | 蜕变(35) 集大成者(13) |
| 极端积极 | 3/3 | 高速成长(194) 成倍增长(30) 几倍(13) |
| 极端消极 | 3/3 | 恐慌(15) |
| 极端消极 | 2/3 | 超跌(27) |
| 条件性 | 3/3 | 寡头(297) 明星(202) |
| 条件性 | 2/3 | 巨头(765) 刷新(19) |

**条件性**指单独出现时不构成断言，只有组成特定搭配才是（如「明星产品」）。
这类词是否收入、按什么搭配规则收入，属于口径选择，需在审核时一并裁定。
...
- **PPMI 阶段没有反向污染计分**（这一条最重要）：扩展跑完后重跑计分，
  `措辞证据.jsonl`、`逐篇指标.json`、`lexicon.json`、`disambiguation.json` 的
  SHA256 与扩展前完全一致——扩展只读词表，不改词表。
...
- **待审核候选尚未裁定**。当前 17 条候选**没有进入词表**，所以上面的结果数字
  是扩展前的状态；裁定并回灌后需重跑计分并重做一次全量 diff。
