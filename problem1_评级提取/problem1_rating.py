...
# 不含“增持”的版本：兜底模式里“增持”必须单独走带股东行为排除的分支
RATING_SRC_NO_ADD = r"(?:强烈|谨慎|审慎)?(?:买入|推荐|强推|中性|持有|减持|卖出|回避|观望)" + SUFFIX
...
FALLBACK_PATTERNS = [
    ...
    re.compile(r"(维持|继续|仍维持|重申)[“”\"']*(?P<r>" + RATING_SRC_NO_ADD + r")(?!机构|投资者|客户|给)"),
    # “增持”单列：排除股东行为（增持至XX%、增持公司股票、增持股份…）
    re.compile(r"(维持|继续|仍维持|重申)[“”\"']*(?P<r>增持" + SUFFIX + r")"
               r"(?!至|股份|计划|比例|了|约|达|超|数|万股|公司|股票|持股|股权)"),
...
# 评级词与“评”之间允许少量字（“的投资评”“投资评”），中间也不能跨句。
TRUNCATED_RATING = re.compile(
    r"(维持|继续|重申|给予|给与|给以|首次)[^。；\n]{0,12}?[“”\"']*(?P<r>" + RATING_SRC + r")"
    r"[”\"' ]*(?:的)?(?:投资)?[”\"' ]*评(?!级|估|价|论|述|审|选|议|注|点|分)")
...
BARE_RATING = re.compile(
    r"(?<!不)(?<!难以)(?<!组合)(?<!板块)(?<!重点)"
    # 前置情态/助动词排除：“长期内仍可推荐”“值得推荐”是倾向表述，不是评级档位
    r"(?<![可会值应宜该需能])"
...
def _clean(text: str) -> str:
    """清洗文本：统一连接符与引号、去掉隐形字符、合并被换行拆散的“评 级”。"""
    # 各种破折号/连字符统一成 ASCII "-"。软连字符(U+00AD)在语料里有两个用途，
    # 都表示"-"：“买入\xadA”= 买入-A（65 处）、“2010\xad2012 年”= 年份区间（71 处）；
    # 早先直接删除它会把档位后缀吞掉（增持-A 变成“增持A”，后缀丢失）
    text = re.sub(r"[\xad‐‑‒–—−－]", "-", text)
    # 其余零宽/不可见字符直接删除
    for ch in ["﻿", "​", "‌", "‍", "\xa0"]:
        text = text.replace(ch, "")
    text = text.replace("''", "”").replace("``", "“")
    # 语料里除 “” 外还夹着 ‚ ‛ ’ ‘ ‟ 「」 等冷门引号（共约 300 处），
    # 不归一化会让“维持‚谨慎推荐‛评[截断]”这类句子漏抓
    text = re.sub(r"[‘’‚‛‟「」]", '"', text)
    text = re.sub(r"评[ \t\r\n]+级", "评级", text)
    return text.strip()
